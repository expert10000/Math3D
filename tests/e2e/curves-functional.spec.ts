...
test.describe("Curves canonical workspace", () => {
  test("orchestrates revision-safe scientific result cards and layered presets", async () => {
    ...
    try {
      ...
      await expect(ctx.page.getByTestId("curve-analysis-plot-series")).toContainText("Plots (2): Curvature κ, Sampling error");
      await expect(ctx.page.getByTestId("curve-analysis-viewport-evidence")).toContainText(/curvature comb, evolute, diagnostic markers.*comb [1-9]\d*/);
      await expect(ctx.page.getByTestId("curve-scalar-plots")).toHaveAttribute("data-series-count", "2");
      await expect(ctx.page.getByTestId("curve-plot-curvature")).toHaveAttribute("data-point-count", /[1-9]\d*/);
      await expect(ctx.page.getByTestId("curve-plot-speed")).toHaveCount(0);
      const cards = ctx.page.getByTestId("curve-result-cards").locator("article");
      await expect(cards).toHaveCount(3);
      const curvatureCard = ctx.page.getByTestId("curve-result-card-preset:curvature");
      await expect(curvatureCard).toContainText("Math3D Curve Core");
      await expect(curvatureCard).toContainText("Uncertainty:");
      await curvatureCard.getByRole("button", { name: "Hide" }).click();
      await expect(ctx.page.getByTestId("curve-analysis-plot-series")).toContainText("Plots (1): Sampling error");
      await expect(ctx.page.getByTestId("curve-analysis-viewport-evidence")).not.toContainText("curvature comb");
      await curvatureCard.getByRole("button", { name: "Show" }).click();
      await curvatureCard.getByRole("button", { name: "Select" }).click();
      await expect(ctx.page.getByTestId("curve-inspector-active-result")).toContainText("Curvature");
      await curvatureCard.getByRole("button", { name: "Frame" }).click();
      await expect(ctx.page.getByTestId("curve-analysis-frame-status")).toContainText("Camera fit includes");
      await curvatureCard.getByRole("button", { name: "Pin" }).click();
      await curvatureCard.getByRole("button", { name: "Save" }).click();
      ...
      await ctx.page.getByRole("checkbox", { name: "Frames", exact: true }).check();
      await expect(ctx.page.getByTestId("curve-analysis-viewport-evidence")).toContainText(/Bishop frame.*glyphs [1-9]\d*/);
      await bishopCard.getByRole("button", { name: "Compare" }).click();
      await curvatureCard.getByRole("button", { name: "Compare" }).click();
      await expect(ctx.page.getByTestId("curve-result-comparison")).toContainText("Common domain");
      await bishopCard.getByRole("button", { name: "Hide" }).click();
      await expect(ctx.page.getByTestId("curve-analysis-viewport-evidence")).not.toContainText("Bishop frame");
      await expect(ctx.page.getByTestId("curve-analysis-viewport-evidence")).toContainText("glyphs 0");
      await bishopCard.getByRole("button", { name: "Show" }).click();
      await curvatureCard.getByRole("button", { name: "Unpin" }).click();
      ...
      await ctx.page.getByRole("textbox", { name: "x(t)", exact: true }).fill("t");
      await ctx.page.getByRole("textbox", { name: "y(t)", exact: true }).fill("t^3");
      await ctx.page.getByTestId("curve-panel-analysis").click();
      ...
      await ctx.page.getByTestId("curve-panel-gallery").click();
      await ctx.page.getByRole("button", { name: /B-spline demo/ }).first().click();
      await ctx.page.getByTestId("curve-panel-analysis").click();
