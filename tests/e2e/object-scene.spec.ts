...
test("Geometry professional shell keeps current workspaces and tools reachable", async () => {
  ...
  try {
    ...
    await expect(page.getByTestId("geometry-characteristic-analysis-controls")).toBeVisible();
    await page.getByTestId("geometry-run-characteristic-analysis").click();
    await expect(page.getByTestId("geometry-characteristic-analysis-result")).toContainText("parabolic");
    await expect(page.getByTestId("geometry-characteristic-layers")).toContainText("display");
    await expect(page.getByTestId("geometry-characteristic-singularities")).toContainText("pole");
    await expect(page.getByTestId("geometry-characteristic-intersections")).toContainText("transverse");
    await expect(page.getByTestId("geometry-promote-characteristic-layer")).toBeVisible();
    await expect(page.getByTestId("geometry-validity-analysis-controls")).toBeVisible();
    await page.getByTestId("geometry-run-validity-analysis").click();
    await expect(page.getByTestId("geometry-native-topology")).toContainText("triangle-independent yes");
    await expect(page.getByTestId("geometry-validity-issues")).toContainText("pole");
    await expect(page.getByTestId("geometry-mesh-health-separation")).toContainText("exact B-rep validity: no");
    await expect(page.getByTestId("geometry-measure-group")).toBeVisible();
    await page.getByTestId("geometry-run-canonical-measurements").click();
    await expect(page.getByTestId("geometry-canonical-measurement-result")).toContainText("measurements");
    await expect(page.getByTestId("geometry-measurement-table")).toContainText("exact-analytic");
    await expect(page.getByTestId("geometry-measurement-sections")).toContainText("iso-u");
    await expect(page.getByTestId("geometry-measurement-provenance")).toContainText("source");
    await expect(page.getByTestId("geometry-sampled-field-controls")).toBeVisible();
    await page.getByTestId("geometry-run-sampled-field").click();
    await expect(page.getByTestId("geometry-sampled-field-progress")).toContainText("complete", { timeout: 15000 });
