...
const outFile = path.join(outDir, "mesh-deferred-analysis-regression.json");
...
function createIsolatedElectronProfile() {
  const profileRoot = mkdtempSync(path.join(os.tmpdir(), "math3d-mesh-deferred-analysis-"));
...
async function main() {
  ...
  const env = {
    ...
    MATH3D_MESH_TRACE_AUTORUN: "deferred-analysis",
  };
  ...
  if (!artifact.ok) {
    console.error(`[mesh-deferred-analysis-regression] FAIL ${outFile}`);
    throw new Error(finishPacket?.error || `Electron autorun failed with code ${exit.code}`);
  }
  console.log(`[mesh-deferred-analysis-regression] PASS ${outFile}`);
  process.exitCode = 0;
