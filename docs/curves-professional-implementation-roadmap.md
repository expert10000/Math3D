...
- The renderer TypeScript project and production renderer build pass.
- The Electron lifecycle journey applies all six analysis presets and asserts real scalar series plus viewport glyph, comb, evolute, marker, control, knot, sample, and tube counts. It verifies that the viewer shell and WebGL canvas remain at least 250 px high before and after every preset, Bishop analysis supplies the active Bishop probe frame, hiding Bishop evidence removes its glyphs, ordinary Frames visibility toggling removes and restores glyphs without invalidating the preset, selection updates the Inspector, framing fits the camera, pinned evidence survives a preset change, saved state persists, and source edits mark the preset stale.
