...
- Samsung `SM-A566B` passed basic physical smoke on older builds. Build `150002` exposed bottom navigation overlap; build `150003` fixed the safe area but failed unsaved Explore scene restore. Build `150004` passed focused exact-build signoff, including USB-free restore. Build `150006` passed ten repeated Samsung launch/Gallery/orbit/quality/worker-health/background cycles, exact-build Files persistence, ten fresh implicit previews over Wi-Fi, and ten invalid endpoint checks. Its owner-observed USB-free Catenoid restore and [exact-build signoff](mobile-device-signoff-150006.json) are approved. The first production/upload key and local and CI verified signed `1.5.0` AABs exist. The [1.5.1 candidate](mobile-1.5.1-release-readiness.md) has new signed binaries, emulator smoke, and verified encrypted USB key backup; exact-build phone checks remain open. See the [historical matrix](mobile-150006-release-matrix.md), [MOB24 history](mobile-mob24-physical-evidence.md), [build/install guide](mobile-android-build-and-install.md), and [Phase 5 checklist](mobile-phase5-stability-checklist.md).
