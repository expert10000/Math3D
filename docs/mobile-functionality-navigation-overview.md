...
| **Display** | Switch render quality among performance, balanced, and sharp; toggle the grid and axes. Both coordinate controls default on and persist across launches. | No lighting, color, bounding-box, wireframe, or advanced rendering controls. |
...
| --- | --- |
| **Local 3D rendering** | Expo GL with React Three Fiber renders explicit and parametric surface previews. The camera uses Z-up coordinates, matching the surface expressions; an adaptive XY grid, colored axes, and a badge with the grid plane's actual Z level and unit spacing give spatial reference for Catenoid and other scenes. Touch camera controls, visibility, selection, opacity, and camera fit/reset are wired to the viewport. |
...
| **Local persistence** | Scene projects use the shared `math3d.scene-project` serializer in the app document directory. Settings save the worker URL, resolution cap, last scene/object, camera orbit, grid/axes visibility, GL state, and selected diagnostics. Mesh previews have a bounded local cache. |
