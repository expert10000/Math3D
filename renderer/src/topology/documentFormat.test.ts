...
describe("topology document format", () => {
  ...
  it("reconstructs structural contracts for legacy v1 cached builds", () => {
    ...
    delete (legacy as { fundamentalGroup?: unknown }).fundamentalGroup;
    ...
    legacy.pipeline = legacy.pipeline.filter((stage) => stage.id !== "validation" && stage.id !== "boundary" && stage.id !== "homology" && stage.id !== "algebra" && stage.id !== "fundamental-group" && stage.id !== "classification");
    ...
    expect(normalized.fundamentalGroup.status).toBe("exact");
    ...
    expect(normalized.pipeline.some((stage) => stage.id === "fundamental-group")).toBe(true);
