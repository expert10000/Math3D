...
const sourceFor = (input: CurveAdapterInput): CanonicalCurveDefinition["source"] => {
  switch (input.representation) {
    ...
    case "polyline": return { familyId: input.sourceLabel ?? "polyline", expressions: input.sourceExpressions, pointCount: input.points.length, points: input.points.map((point) => [...point]) };
