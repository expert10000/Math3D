...
export const CurveWorkerPanel: React.FC<CurveWorkerPanelProps> = ({ definition, points, documentAdapter }) => {
  const fallbackAdapter = useMemo(() => new CurveDocumentAdapter(curveDocumentFromLegacyDefinition(definition)), [definition.fingerprint]);
  const activeDocumentAdapter = documentAdapter ?? fallbackAdapter;
  const adapterRef = useRef(activeDocumentAdapter); adapterRef.current = activeDocumentAdapter;
  const bridgeRef = useRef<CurveAnalysisKernelBridge | null>(null);
  if (!bridgeRef.current) bridgeRef.current = new CurveAnalysisKernelBridge(() => adapterRef.current);
  ...
  return <div data-testid="curve-worker-panel" style={{ marginTop: 8, borderTop: "1px solid #d6deea", paddingTop: 7, display: "grid", gap: 6 }}>
    ...
    <div data-testid="curve-worker-lifecycle" style={{ border: "1px solid #bfdbfe", borderRadius: 6, background: "#eff6ff", padding: 6, display: "grid", gap: 2 }}>
      ...
      {kernelPublication && <span data-testid="curve-worker-kernel-publication">Kernel result: {kernelPublication}</span>}
    </div>
