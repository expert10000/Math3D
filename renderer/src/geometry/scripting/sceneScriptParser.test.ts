...
describe("scene script parser", () => {
  it("parses the supported syntax into typed commands", () => {
    ...
    const clear = result.commands[0] as ClearCommand;
    const add = result.commands[1] as AddObjectCommand;
    const set = result.commands[2] as SetObjectCommand;
    const hide = result.commands[3] as VisibilityCommand;
    const show = result.commands[4] as VisibilityCommand;
    const select = result.commands[5] as SelectCommand;
    const remove = result.commands[6] as DeleteObjectCommand;
